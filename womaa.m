...
function [Best_score, Best_pos, WOA_curve] = womaa(populationSize, Max_iteration, lb, ub, dim, fobj)
...
for t = 1:Max_iteration
    ...
    for i = 1:populationSize
        ...
        adaptive_step_size = step_size * (1 - t / Max_iteration);
        
        X_P1(i, :) = X(i, :) + adaptive_step_size * rand(1, dim) .* (SW - I .* X(i, :)); % Eq(3)
    ...
    for i = 1:populationSize
        ...
        X_P2(i, :) = max(X_P2(i, :), lb);
        X_P2(i, :) = min(X_P2(i, :), ub);
    ...
    for i = 1:populationSize
        ...
        X_P3(i, :) = max(X_P3(i, :), LO_LOCAL);
        X_P3(i, :) = min(X_P3(i, :), HI_LOCAL);
        
        X_P3(i, :) = max(X_P3(i, :), lb);
        X_P3(i, :) = min(X_P3(i, :), ub);
